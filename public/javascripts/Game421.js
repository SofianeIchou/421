...
class Game421 {
  ...
  start(restart) {
    ...
    document
      .querySelectorAll(".token-p1")
      .forEach((el) => insertTokenPlayer(el, 1));
    document
      .querySelectorAll(".token-p2")
      .forEach((el) => insertTokenPlayer(el, 2));
  ...
  addEventOnDices() {
    for (const keyDice in this.dices) {
      if (Object.hasOwnProperty.call(this.dices, keyDice)) {
        ...
        dice.elementHtml.onclick = (ev) =>
          dice.boardToAside(ev, this.getIsPlayingPlayer().id);
      }
    }
  }
  roll() {
      ...
      if (this.gameRound === "charge") {
        ...
        if (currentPlayer.turn === this.powerTurn || isValidateEvent) {
          ...
          if (currentPlayer.turn === this.powerTurn - 1) {
            ...
            gameRoundElement.textContent = `À ${
              currentPlayer.name
            } jet restant ${this.powerTurn - currentPlayer.turn}`;
          }
          setTimeout(() => {
            this.checkNenette();
            this.noshot = false;
  ...
  dechargeGameRound() {
    ...
    if (currentPlayer.combi === "") {
      ...
      if (waitingPlayer.combi === "") {
        ...
        let resultCompare = this.dices.compareCombi(
          currentPlayer,
          waitingPlayer
        );
        let arrTokensPlayerloser = this[`tokensP${resultCompare.loser}Obj`];
  ...
  gameEnd(winnerPlayer) {
    ...
    this.gameRound = "end";
    let scoreOld = Number(
      window.sessionStorage.getItem(`score${winnerPlayer.id}`)
    );
    ...
    if (this.dices.getCombi() === 221) {
      ...
      if (this.gameRound === "charge") {
        setTimeout(() => {
          ...
          if (Token.tokenInPot < nbToken) nbToken = Token.tokenInPot;
          currentPlayer.giveToken(
            nbToken,
            arrTokensPlayerloser,
            arrTokensBoard
          );
          this.addRemovePlayerTokens(currentPlayer.id, nbToken, true);
          if (Token.tokenInPot <= 0)
            return this.startDecharge(currentPlayer.id);
        }, 1500);
      } else if (this.gameRound === "chargeAuto") {
        if (Token.tokenInPot < nbToken) nbToken = Token.tokenInPot;
        arrTokensPlayerWinner = this.tokensBoardObj;
